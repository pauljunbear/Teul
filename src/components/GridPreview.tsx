...
export const GridPreview: React.FC<GridPreviewProps> = ({
  ...
  const renderColumnGrid = (columns: ColumnGridConfig) => {
    const marginPercent =
      columns.marginUnit === 'percent' ? columns.margin : (columns.margin / width) * 100;
    const gutterPercent =
      columns.gutterUnit === 'percent' ? columns.gutterSize : (columns.gutterSize / width) * 100;
  ...
  const renderModuleIntersections = () => {
    ...
    const colMarginPx =
      ((cols.marginUnit === 'percent' ? cols.margin : (cols.margin / width) * 100) / 100) * width;
    const colGutterPx =
      ((cols.gutterUnit === 'percent' ? cols.gutterSize : (cols.gutterSize / width) * 100) / 100) *
      width;
    const availableWidth = width - colMarginPx * 2;
    const colWidth = (availableWidth - colGutterPx * (cols.count - 1)) / cols.count;

    const rowMarginPx =
      ((rows.marginUnit === 'percent' ? rows.margin : (rows.margin / height) * 100) / 100) * height;
    const rowGutterPx =
      ((rows.gutterUnit === 'percent' ? rows.gutterSize : (rows.gutterSize / height) * 100) / 100) *
      height;
